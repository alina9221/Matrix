#include <src/matrix.h>
#include <iostream>
#include "matrix.h"

using namespace math;

real &Matrix::operator()(int row, int col)
{
    if (row >= this->rows_)
    {
        std::cerr << "Matrix: row number out of bounds" << std::endl; //Не совпадает кол-во строк матриц
        // return 0;
    }
    if (col >= this->cols_)
    {
        std::cerr << "Matrix: col number out of bounds" << std::endl; //Не совпадает кол-во столбцов матриц
        // return 0;
    }

    int pos{0};

    pos = cols_ * row + col; //Кол-во элементов матрицы

    return this->mvec_.at(pos);
}

real Matrix::operator()(int row, int col) const
{
    if (row >= this->rows_)
    {
        std::cerr << "Matrix: row number out of bounds" << std::endl;
        // return 0;
    }
    if (col >= this->cols_)
    {
        std::cerr << "Matrix: col number out of bounds" << std::endl;
        // return 0;
    }

    int pos{0};

    pos = cols_ * row + col;

    return this->mvec_.at(pos);
}

void Matrix::print() // Преобразрвание двумерной матрицы в одномерную
{
    for (int i = 0; i < this->rows_; ++i)
    {
        for (int j = 0; j < this->cols_; ++j)
        {
            std::cout << this->mvec_.at(cols_ * i + j) << " ";
        }
        std::cout << std::endl;
    }
}

Matrix math::operator+(const Matrix &A, const Matrix &B)
{
    if ((A.cols_ != B.cols_) || (A.rows_ != B.rows_)) //Проверка совпадения размеров матриц
    {
        std::cerr << "Matrix: Matrices can't be added!" << std::endl;
        return Matrix(0, 0);
    }

    Matrix M(A.rows_, A.cols_); //Сложение матриц

    for (int i = 0; i < M.mvec_.size(); ++i)
    {
        M.mvec_.at(i) = A.mvec_.at(i) + B.mvec_.at(i);
    }

    return M;
}

Matrix math::operator-(const Matrix &A, const Matrix &B) 
{
    if ((A.cols_ != B.cols_) || (A.rows_ != B.rows_)) //Проверка совпадения размеров матриц
    {
        std::cerr << "Matrix: Matrices can't be subtracted!" << std::endl;
        return Matrix(0, 0);
    }

    Matrix M(A.rows_, A.cols_);  //Вычитание матриц

    for (int i = 0; i < M.mvec_.size(); ++i)
    {
        M.mvec_.at(i) = A.mvec_.at(i) - B.mvec_.at(i);
    }

    return M;
}

Matrix math::operator*(const Matrix &A, const Matrix &B) 
{
    if (A.cols_ != B.rows_) //Проверка совпадения столбцов и строк матриц
    {
        std::cerr << "Matrix: Matrices can't be multiplied!" << std::endl;
        return Matrix(0, 0);
    }

    Matrix M(A.rows_, B.cols_);

    for (int pos = 0; pos < M.mvec_.size(); ++pos) //Умножение матриц
    {
        int row = (int)std::floor(pos / M.cols_);
        int col = pos - row * M.cols_;

        for (int k = 0; k < A.cols_; ++k)
        {
            M.mvec_.at(pos) = A(row, k) * B(k, col);
        }
    }

    return M;
}


Matrix& Matrix::operator+=(const Matrix& other) //Сложение матриц
{
    if (rows_ != other.rows_ || cols_ != other.cols_) {
        throw std::invalid_argument("Matrix: Matrices can't be added!");
    }
    
    for (size_t i = 0; i < mvec_.size(); ++i) {
        mvec_[i] += other.mvec_[i];
    }
    
    return *this;  
}

Matrix& Matrix::operator-=(const Matrix& other) //Сложение матриц
{
    if (rows_ != other.rows_ || cols_ != other.cols_) {
        throw std::invalid_argument("Matrix: Matrices can't be subtracted!");
    }
    
    for (size_t i = 0; i < mvec_.size(); ++i) {
        mvec_[i] -= other.mvec_[i];
    }
    
    return *this;  
}

Matrix& Matrix::operator*=(double scalar)
{
    for (size_t i = 0; i < mvec_.size(); ++i) 
    {
        mvec_[i]*= scalar;
    }    
    return *this;
}

 
  std::istream& math::operator>>(std::istream &is, Matrix &matrix) {
    for (int i = 0; i < matrix.rows_; ++i) {
        for (int j = 0; j < matrix.cols_; ++j) {
            is >> matrix.mvec_[matrix.cols_ * i + j]; // Правильный доступ
        }
    }
    return is;
}

std::ostream& math::operator<<(std::ostream &os, const Matrix &matrix) {
    for (int i = 0; i < matrix.rows_; ++i) {
        for (int j = 0; j < matrix.cols_; ++j) {
            os << matrix.mvec_[matrix.cols_ * i + j] << " "; // Правильный доступ
        }
        os << std::endl;
    }
    return os;
}
